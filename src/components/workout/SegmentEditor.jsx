...
export function SegmentEditor({ segment, onChange, onDelete, depth = 0 }) {
  ...
  if (segment.type === 'repeat') {
    const updateNestedSegment = (index, updated) => {
      const newSegments = [...segment.segments];
      newSegments[index] = updated;
      onChange({ ...segment, segments: newSegments });
    };

    const deleteNestedSegment = (index) => {
      const newSegments = segment.segments.filter((_, i) => i !== index);
      onChange({ ...segment, segments: newSegments });
    };

    const addNestedSegment = (type) => {
      const newSegment =
        type === 'block'
      ...
      onChange({ ...segment, segments: [...segment.segments, newSegment] });
    };
    ...
    return (
      <div className={`${baseClasses} ${typeColors.repeat}`}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex items-center gap-1">
            <input
              type="number"
              value={segment.count}
              onChange={(e) => updateField('count', Math.max(1, parseInt(e.target.value, 10) || 1))}
              className="w-16 px-2 py-1 bg-gray-700 rounded text-center"
              min="1"
              max="100"
            />
        ...
        <div className="ml-4 space-y-2">
          {segment.segments.map((nested, index) => (
            <SegmentEditor
              key={nested.id}
              segment={nested}
