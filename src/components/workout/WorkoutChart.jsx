...
import { flattenWorkout, calculateTotalDuration } from '../../utils/workoutUtils.js';

export function WorkoutChart({ workout, height = 120, highlightTime = null }) {
  const { points, maxPower, totalDuration } = useMemo(() => {
    if (!workout || workout.segments.length === 0) {
      return { points: [], maxPower: 200, totalDuration: 0 };
    }
  ...
  const xScale = (time) => padding.left + (time / totalDuration) * innerWidth;
  const yScale = (power) => padding.top + innerHeight - (power / maxPower) * innerHeight;
