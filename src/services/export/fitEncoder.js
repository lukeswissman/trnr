...
// src/services/bluetooth/ftms.js.
...
const BASE_TYPE_SINT16 = 0x83;
const BASE_TYPE_STRING = 0x07;
...
function fitCrc(data, start, end) {
  let crc = 0;
...
const FIELD = (num, size, type) => [num, size, type];

/**
 * Write a FIT definition message.
 * @param {FitBuffer} buf
 * @param {number} localMesg - Local message type (0-15)
 * @param {number} globalMesg - Global message number
 * @param {Array<[number,number,number]>} fields - Array of [fieldDefNum, size, baseType]
 */
function writeDefinition(buf, localMesg, globalMesg, fields) {
 ...
 * @param {number} localMesg - Local message type (0-15)
 */
function writeDataHeader(buf, localMesg) {
 ...
 * @returns {Uint8Array} FIT file bytes
 */
export function encodeFit(recording) {
